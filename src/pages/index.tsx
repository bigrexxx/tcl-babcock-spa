...
export default function IndexPage() {
  ...
  const { data: teamData } = useQuery({ queryKey: ["team-public"], queryFn: listTeamMembers });
  ...
  const filteredCommittees = useMemo(() => {
    ...
    if (q) {
      list = list.filter(
        (c) => c.name.toLowerCase().includes(q) || c.tagline.toLowerCase().includes(q) || c.desc.toLowerCase().includes(q)
      );
    }
    if (activeFilter !== "All") {
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          ...
          <div className="hero-image">
            <img src={images.hero} alt="TCL Babcock students collaborating" width={1600} height={1024} loading="eager" />
          </div>
        ...
        <div className="container">
          ...
          <div className="comm-search-wrap reveal">
            <div className="comm-search">
              ...
              <input type="text" placeholder="Search committees..." value={query} onChange={(e) => setQuery(e.target.value)} />
              {query && <button className="comm-search-clear" onClick={() => setQuery("")} aria-label="Clear search">✕</button>}
            </div>
            <div className="comm-filters">
              {categories.map((cat) => (
                <button key={cat} className={`comm-filter-chip ${activeFilter === cat ? "active" : ""}`} onClick={() => setActiveFilter(cat)}>{cat}</button>
              ))}
            </div>
            <div className="comm-results-count">Showing {filteredCommittees.length} of {COMMITTEES.length} committees</div>
          </div>
          <div className="c-grid">
            {filteredCommittees.map((c) => (
              <Link key={c.id} to={`/committees/${c.id}`} className="comm-card reveal">
          ...
          {filteredCommittees.length === 0 && (
            <div className="comm-empty reveal">
              ...
              <button className="btn-secondary" onClick={() => { setQuery(""); setActiveFilter("All"); }}>Clear filters</button>
            </div>
          )}
          <div className="structure-note reveal">
            <strong>Reporting Chain:</strong> Committee Members → Directors → Executive Council → President.
          </div>
        </div>
      </section>
            ...
            {executives.map((p) => (
              <div key={p.id} className="tcard reveal" onClick={() => setProfileMember(p)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter") setProfileMember(p); }}>
                <div className="avatar">
                  {p.photo_url ? <img src={p.photo_url} alt={p.name} loading="lazy" /> : <span>{p.name.split(" ").map((s) => s[0]).join("").slice(0, 2)}</span>}
                </div>
                <h5>{p.name}</h5>
        ...
        <div className="profile-overlay" onClick={() => setProfileMember(null)}>
          <div className="profile-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="profile-photo">
              {profileMember.photo_url ? <img src={profileMember.photo_url} alt={profileMember.name} /> : <span>{profileMember.name.split(" ").map((s: string) => s[0]).join("").slice(0, 2)}</span>}
            </div>
            ...
            {profileMember.bio ? <p className="profile-bio">{profileMember.bio}</p> : <p className="profile-bio-empty">No bio yet.</p>}
          </div>
        </div>
      )}
