...
export default function StudioPage() {
  ...
  useEffect(() => { document.title = "TCL Studios 25 — Book a Session"; }, []);
    ...
    <>
      ...
      <div className="page-wrap">
        <div className="studio-hero-img">
          <img src={images.studio} alt="TCL Studios 25" width={1600} height={1024} />
        </div>
        <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
          ...
          <p style={{ color: "var(--muted)", maxWidth: 560, margin: "0 auto" }}>
            A working studio for portraits, brand shoots, reels, podcasts, and graduation sessions.
          </p>
        </div>
        ...
        <div className="pkg-grid">
          {PACKAGES.map((p) => (
            <button key={p.id} type="button" className={"pkg-card" + (p.featured ? " featured" : "") + (pkg === p.id ? " selected" : "")} onClick={() => setPkg(p.id)}>
        ...
        <div className="addon-grid">
          {ADDONS.map(([n, p]) => (
            <button key={n} type="button" className={"addon-chip" + (selectedAddons.includes(n) ? " selected" : "")}
              onClick={() => setSelectedAddons((prev) => prev.includes(n) ? prev.filter((x) => x !== n) : [...prev, n])}>
        ...
        {done ? (
          <div className="wizard-card success-card">
            ...
            <p style={{ color: "var(--muted)", margin: "1.2rem auto", maxWidth: 420 }}>We'll email a confirmation and prep checklist within an hour.</p>
              ...
              <div className="slot-grid">
                ...
                {SLOTS.map((s) => (
                  <button key={s} type="button" className={"slot" + (slot === s ? " selected" : "") + (BOOKED.has(s) ? " booked" : "")}
                    disabled={BOOKED.has(s) || slotsLoading} onClick={() => setSlot(s)}>{s}</button>
                ))}
              </div>
            </div>
