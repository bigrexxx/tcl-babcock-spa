...
export default function RegisterPage() {
  ...
  useEffect(() => {
    document.title = "Join TCL Babcock — Membership Application";
    const s = loadState();
  ...
  const submit = async () => {
    ...
    try {
      ...
      Object.entries(values).forEach(([k, v]) => { if (!generalIds.has(k)) answers[k] = v; });
        ...
        {done ? (
          <div className="wizard-card success-card">
            ...
            <p style={{ color: "var(--muted)", margin: "1.5rem auto", maxWidth: 420 }}>
              We'll review your application and email you within 7 days.
            </p>
          ...
          <>
            ...
            {step === 1 && (
              <div className="wizard-card">
                ...
                <p className="sub">Pick the team that matches your craft.</p>
                <div className="committee-pick-grid">
                  {COMMITTEES.map((c) => (
                    <button key={c.id} type="button" className={"pick-card" + (committeeId === c.id ? " selected" : "")} onClick={() => setCommitteeId(c.id)}>
                ...
                <div className="wizard-actions">
                  <span />
                  <button className="btn-primary" disabled={!committeeId} onClick={() => setStep(2)}>Continue →</button>
                </div>
              </div>
            )}
            {step === 2 && committee && (
              <div className="wizard-card">
                ...
                {GENERAL_Q.map((q) => <Field key={q.id} q={q} value={values[q.id]} onChange={(v) => setVal(q.id, v)} error={errors[q.id]} />)}
                <div className="section-title">{committee.icon} {committee.name} questions</div>
                {committee.questions.map((q) => <Field key={q.id} q={q} value={values[q.id]} onChange={(v) => setVal(q.id, v)} error={errors[q.id]} />)}
                <div className="wizard-actions">
                  <button className="btn-secondary" onClick={() => setStep(1)}>← Back</button>
                  <button className="btn-primary" onClick={() => { if (validateStep2()) setStep(3); }}>Continue →</button>
                </div>
              </div>
            )}
            {step === 3 && committee && (
              <div className="wizard-card">
                ...
                <div className="committee-banner">
                  ...
                  <div><strong>{committee.name}</strong><div style={{ color: "var(--muted)", fontSize: "0.82rem" }}>{committee.tagline}</div></div>
                </div>
        ...
        {(q as any).options.map((o: string) => {
          ...
          return (
            <label key={o} className={"opt" + (checked ? " checked" : "")}>
              <input type="checkbox" checked={checked} onChange={() => onChange(checked ? arr.filter((x) => x !== o) : [...arr, o])} />
              <span>{o}</span>
            </label>
          );
        })}
  ...
  if (q.type === "scale") {
    ...
    return (
      <div className="scale-row">
        ...
        {nums.map((n) => <button key={n} type="button" className={"scale-btn" + (value === n ? " active" : "")} onClick={() => onChange(n)}>{n}</button>)}
        <span className="scale-label">{sq.maxLabel}</span>
      </div>
    );
  }
  return <input type={q.type} value={value || ""} onChange={(e) => onChange(e.target.value)} />;
}
