...
export function OverviewTab({ onJump }: { onJump: (t: TopTab) => void }) {
  ...
  const { data: appData, isLoading: aL } = useQuery({ queryKey: ["admin-apps"], queryFn: adminListApplications, refetchInterval: 30000 });
  const { data: bkData, isLoading: bL } = useQuery({ queryKey: ["admin-bookings"], queryFn: adminListBookings, refetchInterval: 30000 });
